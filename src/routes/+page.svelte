<script>
	...
	const startQuiz = async () => {
		...
		// const resp = await fetch('https://opentdb.com/api.php?amount=10&category=18&type=multiple');
	...
	const saveScore = ({ detail: userName }) => {
		...
		const arr = localStorage.getItem('board') ? JSON.parse(localStorage.getItem('board')) : [];
